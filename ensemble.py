...
if __name__ == "__main__":
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            npz_data = np.load('./data/' + 'ntu/' + 'NTU60_CS_ori.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in arg.dataset:
            npz_data = np.load('./data/' + 'ntu/' + 'NTU60_CV_ori.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    else:
    ...
    if arg.CoM_1:
        with open(os.path.join(arg.main_dir, 'x_view/joint_com1_no_body/', 'epoch1_test_score.pkl'), 'rb') as r1:
            r1 = list(pickle.load(r1).items())
        with open(os.path.join(arg.main_dir, 'x_view/bone_com1_no_body/', 'epoch1_test_score.pkl'), 'rb') as r2:
            r2 = list(pickle.load(r2).items())
        dir_cnt += 2

    if arg.CoM_2:
        with open(os.path.join(arg.main_dir, 'x_view/joint_com2_no_body/', 'epoch1_test_score.pkl'), 'rb') as r3:
            r3 = list(pickle.load(r3).items())
        with open(os.path.join(arg.main_dir, 'x_view/bone_com2_no_body/', 'epoch1_test_score.pkl'), 'rb') as r4:
            r4 = list(pickle.load(r4).items())
        dir_cnt += 2
        
    if arg.CoM_21:
        if 'ntu' in arg.dataset:
            with open(os.path.join(arg.main_dir, 'x_view/joint_com21_no_body/' 'epoch1_test_score.pkl'), 'rb') as r5:
                r5 = list(pickle.load(r5).items())
            with open(os.path.join(arg.main_dir, 'x_view/bone_com21_no_body/', 'epoch1_test_score.pkl'), 'rb') as r6:
